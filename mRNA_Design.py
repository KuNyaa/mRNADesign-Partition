...
def execute_mrna_design(protein, output_dir, run_number, args, progress_tracker, lock):
    ...
    # Execute the command and redirect both stdout and stderr to the output file
...
def run_mrna_design(args):
    ...
    for seq_id, protein in records:
        ...
        for run_file in sorted(os.listdir(log_dir)):
            # Skip if it's not a text file (e.g., .DS_Store files)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run mRNA Design on protein fasta file.')
